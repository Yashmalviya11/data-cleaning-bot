...
def detect_objects(image_path):
    ...
    for result in results:
        for box in result.boxes:
            ...
            if cls_id in OBJECT_NAMES and conf > CONFIDENCE_THRESHOLD:
...
# Function to generate furniture placement suggestions
def generate_suggestions(room_objects, reference_objects):
    ...
    for obj_name, ref_center in reference_positions.items():
        if obj_name in room_positions:
            ...
            suggestions.append(f"➕ Add a {obj_name} to match the reference layout")
...
def annotate_image(image_path, detected_objects, suggestions, output_path):
    ...
    for obj in detected_objects:
        ...
        draw.text((x1, y1 - 10), f"{obj['name']} ({obj['confidence']:.2f})", fill="red")
