...
os.environ["REPLICATE_API_TOKEN"] = "your_actual_api_key_here"
...
class InteriorDesignApp(Form):
    ...
    def __init__(self, owner):
        ...
        self.SetProps(Caption="AI Interior Design Remodel + Replicate API", OnShow=self.__form_show,
        ...
        self.layout_top.SetProps(Parent=self, Align="Top", Height="50",
                                 Margins=Bounds(RectF(3, 3, 3, 3)))

        self.layout_left = Layout(self)
        self.layout_left.SetProps(Parent=self, Align="Left", Width="300",
                                  Margins=Bounds(RectF(3, 3, 3, 3)))

        # Label to prompt the user for image upload
        self.prompt_label = Label(self)
        self.prompt_label.SetProps(Parent=self.layout_top, Align="Client",
                                   Text="Enter the prompt for your new room and then Select an Image of the room:",
                                   Position=Position(PointF(20, 20)), Margins=Bounds(RectF(3, 3, 3, 3)))

        # Memo control for entering the interior design prompt
        self.prompt_memo = Memo(self)
        self.prompt_memo.SetProps(Parent=self.layout_left, Align="Top", Height=60,
                                  Text="Enter design prompt here...", Margins=Bounds(RectF(3, 3, 3, 3)))

        # Button to trigger file upload
        self.upload_button = Button(self)
        self.upload_button.SetProps(Parent=self.layout_top, Align="Right", Text="Select Image",
                                    Position=Position(PointF(290, 18)), Width=120, OnClick=self.__upload_image,
                                    Margins=Bounds(RectF(3, 3, 3, 3)))
        ...
        self.img_original.SetProps(Parent=self.layout_left, Align="Client",
                                   Position=Position(PointF(20, 60)), Width=300, Height=300,
                                   Margins=Bounds(RectF(3, 3, 3, 3)))

        # Image control for the remodeled image display
        self.img_remodel = ImageControl(self)
        self.img_remodel.SetProps(Parent=self, Align="Client",
                                  Position=Position(PointF(340, 60)), Width=300, Height=300,
                                  Margins=Bounds(RectF(3, 3, 3, 3)))

        # Status bar at the bottom
        self.status_bar = Label(self)
        self.status_bar.SetProps(Parent=self, Align="Bottom", Text="Status: Ready", Height=30,
                                 Margins=Bounds(RectF(3, 3, 3, 3)))
    ...
    def __form_close(self, sender, action):
        self.timer.Enabled = False
        action.Value = "caFree"  # Correct way to modify event action
    ...
    def __on_timer_tick(self, sender):
        ...
        try:
            ...
            logs = self.prediction.logs if self.prediction.logs else ""
